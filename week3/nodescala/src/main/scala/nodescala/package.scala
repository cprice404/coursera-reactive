...
//    def always[T](value: T): Future[T] = Future(value)
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      ...
      p.success(Nil)
      fs.foldRight( p.future )( (f:Future[T], acc: Future[List[T]]) => {
        for { t <- f; ts <- acc } yield t :: ts
      })
    }
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      val p = Promise[T]()
      fs.foreach(f => {
        f onComplete { case tryValue => p.tryComplete(tryValue) }
      })
      p.future
    }
    ...
    def delay(t: Duration): Future[Unit] = async {
      try {
        Await.ready(never, t)
        assert(false, "Future.never should never complete!")
      } catch {
        case t: TimeoutException => Unit // ok!
      }
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
      val p = Promise[S]
      f onComplete { case _ => p.complete(Try(cont(Try(f.now))))}
      p.future
    }
